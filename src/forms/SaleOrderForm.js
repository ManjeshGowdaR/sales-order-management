...
const SaleOrderForm = ({
  ...
  useEffect(() => {
    if (initialData && initialData.product) {
      setSelectedProduct(
        products.find((product) => product.id === Number(initialData.product))
      );
    }
  }, [initialData]);
  ...
  const handleFormSubmit = async (data) => {
    try {
      ...
      if (data.paid === "true") {
        const index = activeSaleOrders.findIndex(
  ...
  return (
    <form
      onSubmit={
        readOnly ? (e) => e.preventDefault() : handleSubmit(handleFormSubmit)
      }
    >
              ...
              <Flex alignItems="center">
                <FormControl flex="1" ml={2} isReadOnly={readOnly}>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleSellingRateChange(index, e.target.value)
                    }
                  />
                </FormControl>
      ...
      <FormControl>
        ...
        <Select {...register("paid", { required: true })} isDisabled={readOnly}>
